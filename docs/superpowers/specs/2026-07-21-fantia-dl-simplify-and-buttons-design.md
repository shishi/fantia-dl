...
4. **zip 経路の資源管理は無傷**: Port(start→chunk*→end)、offscreen document、blob URL の
   ...
   **ただし 2 点だけ変更する**:
   ...
   - **zip の直列化と非同期圧縮(adversarial レビュー round21 指摘)**: バジェットは
     ...
     (2 件目以降のギャラリー zip はキューに積み、順に処理する。in-flight メモリは
     **タブごとに**最大 1 バジェットに有界。複数タブを開けば `タブ数 × バジェット` に
     なる ―― これはタブごとの明示的なユーザー操作に比例するため受容する(round22 指摘の
     明示化。拡張全体で 1 本に絞るには zip を background へ移す必要があり、スコープ外))、
     (b) **`zipSync` を fflate の非同期 `zip()`(worker ベース)に置き換え**、圧縮中も
     ページの操作性を保つ。fanbox-dl 式の「zip を background へ移す」全面移植は本変更の
     スコープ外(実行位置は変えず、並行度と同期性だけを直す)。
